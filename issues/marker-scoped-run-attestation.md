...
- `src/bin/xdu.rs:111` — `clear_completion_marker(outdir)?` runs on **every** crawl, including
  `xdu -p onepartition`. The clear is not filtered by `partition_filter`.
- `src/bin/xdu.rs:713` — `write_completion_marker(&outdir, &completion_marker_contents(&stats, …))`
...
GOAL. P9 left a `// Known limitation:` comment at the write site (`xdu.rs:705`) rather than smuggling
