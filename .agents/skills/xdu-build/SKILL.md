...
name: xdu-build
description: >-
  Resume and execute an xdu feature's phased roadmap. Discovers spec/{slug}/TECH.md from the
...
Invoke `/xdu-build` on a feature/fix branch whose `spec/{slug}/TECH.md` exists, to execute the next
...
- FSM: resolved in **Step 1** by running `uv run --with pyyaml python .agents/factory/bin/next_phase.py spec/{slug}/TECH.md` (a load-time injection can't strip the branch prefix to form `{slug}`).
...
- `no-pause` → continue past the natural phase-boundary stop (use sparingly). Deprecated alias:
  `skip review` — accepted, but note it does **not** skip `/xdu-review`.
...
- **Verify by driving the CLI, not just tests.** Run the phase's `verify:` command; for behavior,
  ...
  real index. **Exit 0 is necessary but not sufficient** — assert a concrete post-condition (a row
  count / a known stdout token / files actually gone-or-kept); a run that "completed" but left the
  wrong index or deleted the wrong files is a FAIL. A red gate is a STOP condition — do not mark the
  phase done or advance state.
...
- **Circuit breaker (durable).** Every red verify gate is recorded on file via
  ...
  builds (unknowns unresolved), **stop-and-re-shape**: STOP and recommend `/xdu-plan` (or human
  input) rather than looping. Respect the appetite.
- **`main` is off-limits; never push, squash, force-push, or open PRs** — that is
  `/xdu-publish`. **No `Co-Authored-By` trailer** (attribution lives in the PR body, not the commit).
...
2. Resolve `{slug}` from the branch, then run
   `uv run --with pyyaml python .agents/factory/bin/next_phase.py spec/{slug}/TECH.md` and read its
   output. If it errored or warned of drift, reconcile (`set_phase.py --current …`) or STOP and
   report. `cargo build` if `Cargo.toml`/dependencies changed.
...
drive for behavior). If implementation reveals a real correction to `TECH.md`/`PLAN.md`, amend
`TECH.md` (Step 5 rules); if it reveals a `GOAL.md` contradiction, STOP and escalate.

### Step 4 — Verify gate
Run the phase's `verify:` command (plus any CLI drive). "Green" means the **asserted post-condition
held** — the observed output (row counts, files gone-or-kept, a known token) is correct — not merely that the
command exited 0. Green → proceed. Red → STOP; do not mark done or advance state; record the failure
(`uv run --with pyyaml python .agents/factory/bin/set_phase.py spec/{slug}/TECH.md --phase {id} --record-attempt
...
hand-fixed a command this skill gave (wrong flag/path, a bare `python3`/`uv run python` that should be
`uv run --with pyyaml python`, unquoted YAML); a genuinely ambiguous instruction; a verify gate that
passed/failed misleadingly (e.g. "exit 0" hid the wrong index / wrong files deleted); an
...
(recurrence across phases is exactly the signal `/xdu-harness` acts on); a fix that would weaken a
non-negotiable gate (tests, the CLI-drive verify, an `invariants.md` item) is `severity=high` and must
say so. **Records only** — `/xdu-harness` applies fixes later, human-reviewed. Use the next unused `F#`;
always write `status=open`; append the finding as a section **outside** any code fence:
```markdown
## F<n> — <one-line title>
`origin=xdu-build:{id} severity=<high|medium|low> category=<instruction|steering|tooling|template|missing-guidance> status=open target=<best-guess file>`
...
```
`xdu-build` is **the richest source** and runs **per phase across separate invocations** — appending to
...
output — the same house style as `xdu-feature`/`xdu-plan`. There is **no `WIP:` prefix**: every branch
commit is squashed into the single PR-title commit at `xdu-publish`, so subjects only need to read well
...
the FSM is fully `done` (`status: in_review`), recommend a **clean-session** `/xdu-review`.

## Examples

- `/xdu-build` — next incomplete phase, run its verify, one clean `[category]` commit, stop + report.
- `/xdu-build status` — FSM state + last commit; no work.
- `/xdu-build dry run phase P4` — report what P4 would do (items, files, verify, commit); no edits.
- `/xdu-build through P3` — run each incomplete phase up to P3, one commit each, stopping at boundaries.
...
- This skill never ships to `main`, squashes, or force-pushes — that's `/xdu-publish`. Out
  of scope (`merge`, `push`, `open PR`, `bump version`) → STOP and point at `/xdu-publish`.
