...
"""Shared helpers for the xdu software-factory FSM scripts.
...
Requires PyYAML. xdu is a Rust project with no Python environment of its own, so
the scripts are meant to be run as
``uv run --with pyyaml python .agents/factory/bin/<script>.py`` — ``uv`` provides
PyYAML on demand (see ``.agents/factory/portability.md``).
"""
...
try:
    ...
    raise SystemExit(
        "PyYAML is required. Run these scripts via `uv run --with pyyaml python "
        ".agents/factory/bin/<script>.py` so PyYAML is provided on demand."
    ) from exc
