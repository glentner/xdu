...
default); ``xdu-publish`` and ``/xdu-harness`` read them through this script rather
...
Usage:
    uv run --with pyyaml python .agents/factory/bin/meta_status.py spec/<slug>/META.md
    uv run --with pyyaml python .agents/factory/bin/meta_status.py spec/<slug>/META.md --status open
    uv run --with pyyaml python .agents/factory/bin/meta_status.py spec/<slug>/META.md --severity high --id F1 F3
