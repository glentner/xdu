...
phases:
  - id: P1
    ...
    hammerable: false
    hill: uphill
    verify: "cargo test --test rm_tests"
  - id: P2
    name: "Second slice"
    status: pending
    satisfies: [R2, R3]
    ...
    hammerable: true
    hill: uphill
    verify: ".agents/factory/bin/temp_index.sh sh -c 'xdu-find -i \"$XDU_INDEX\" -u alice --count'"
review:
...
- [ ] <concrete step>
- [ ] <concrete step>
- **Verify:** `cargo test --test rm_tests` (and/or drive the CLI in a `temp_index`).
- **Touches:** `src/…`, `doc/*.scd`.
