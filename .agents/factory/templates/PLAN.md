...
concrete files under `src/…`. Be specific enough to build from, not so specific it
duplicates the code.>
...
**before** research and **again** after this design was drafted. List every load-bearing invariant
this change touches and confirm compliance.
...
each phase's `verify:` command in `TECH.md`). Prefer driving the xdu CLIs
(`xdu`/`xdu-find`/`xdu-rm`/`xdu-view`) in a throwaway index
(`.agents/factory/bin/temp_index.sh sh -c "…"`) over unit tests alone.
